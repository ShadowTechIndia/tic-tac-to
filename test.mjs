...
function runTests() {
  ...
  // Test 2: FIFO Queue Deletion in LocalGame (No immediate wins)
  const game = new LocalGame('pass-and-play');

  game.makeMove(0); // X plays 0
  game.makeMove(1); // O plays 1
  game.makeMove(3); // X plays 3
  game.makeMove(4); // O plays 4
  game.makeMove(2); // X plays 2
  game.makeMove(5); // O plays 5

  // Current Board:
  // X at 0, 3, 2 (Active Marks: 3)
  // O at 1, 4, 5 (Active Marks: 3)
  ...
  console.assert(game.board[0] === 'X' && game.board[1] === 'O', 'Test 2 Failed: Board state mapping incorrect.');

  // X plays a 4th mark at 7.
  // Oldest mark for X was at 0, so it should be deleted.
  const res = game.makeMove(7);
  ...
  console.assert(game.board[7] === 'X', 'Test 2 Failed: New mark at 7 was not placed.');
  console.assert(game.moves.X.length === 3, 'Test 2 Failed: X should still have maximum of 3 active marks.');
  console.assert(game.moves.X.join(',') === '3,2,7', `Test 2 Failed: X moves queue did not shift correctly. Found: ${game.moves.X.join(',')}`);
